// API
import { youtubeApi } from '../../core/api/index';
...
export const videoListActions = {
   ...
   * @returns {Function}
   */
  handleVideoSelection: (videoDetails, videoSource, partyId) => {
    return async function (dispatch) {
      // Create a new party
      dispatch(partyActions.createParty(videoDetails, videoSource, partyId));
    };
  },
  ...
  loadYoutubeVideos: (query, videoType = 'any') => {
    return async function (dispatch) {
      try {
        ...
        const youtubeMovies = await youtubeApi.fetchYoutubeSearchResults(
          query,
          videoType
        );
