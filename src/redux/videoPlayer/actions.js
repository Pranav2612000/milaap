...
export const videoPlayerActions = {
  ...
  handleMaximizeBtnPressed: (playerCurrentlyMaximized, videoPlayerElem) => {
    return async function (dispatch) {
      ...
      dispatch(
        videoPlayerActions.setPlayerMaximizedState(!playerCurrentlyMaximized)
      );
    };
  },
...
const debouncedPlayerStateChangeHandler = debounce(
  (dispatch, userVideoPlayerState) => {
    ...
    dispatch(
      userActions.emitClientReadyStateToServer({
        clientIsReady,
        timeInVideo
      })
    );
