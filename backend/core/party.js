...
const { generalUtils, socketUtils, messageUtils } = require('../utils');

// Constants
const { ACTION_TYPES, serverUserName } = require('../core/constants');
...
export const party = {
  ...
  getPartyById: (partyId) => {
    return cache.parties.find(
      (activeParty) => activeParty.partyId && activeParty.partyId === partyId
    );
  },
  ...
  createParty: (io, socket, videoDetails) => {
    ...
    var partyId = videoDetails.partyId;
  ...
  getSelectedVideoForParty: (partyId) => {
    ...
    return partyForId && partyForId.selectedVideo ? partyForId.selectedVideo : {};
  },
  ...
  getVideoPlayerForParty: (partyId) => {
    const partyForId = party.getPartyById(partyId);
    return partyForId && partyForId.videoPlayer ? partyForId.videoPlayer : {};
  },
  ...
  getMessagesInParty: (partyId) => {
    const partyForId = cache.parties.find(
      (activeParty) => activeParty.partyId === partyId
    );

    return partyForId && partyForId.messagesInParty
      ? partyForId.messagesInParty
      : [];
  },
  ...
  sendPartyDetailsToClient: (socket, partyId) => {
    ...
    socketUtils.emitActionToClient(
      socket,
      ACTION_TYPES.SET_SELECTED_VIDEO,
      videoForParty
    );
    ...
    if (videoPlayerForParty.timeInVideo !== 0) {
      socketUtils.emitActionToClient(
        socket,
        ACTION_TYPES.SET_CLIENT_PLAYER_STATE,
        videoPlayerForParty
      );
    }
    ...
    socketUtils.emitActionToClient(
      socket,
      ACTION_TYPES.SET_USERS_IN_PARTY,
      usersInParty
    );
    ...
    socketUtils.emitActionToClient(
      socket,
      ACTION_TYPES.PARTY_MESSAGE_RECEIVED,
      messagesInParty
    );
  },
  ...
  notifyPartyOfNewlyJoinedUser: (io, partyId, userName) => {
    ...
    const userJoinedMessage = messageUtils.generateUserJoinedMessage(
      userName,
      partyId,
      serverUserName
    );
  ...
  emitVideoPlayingMessageToParty: (io, socket, partyId) => {
    ...
    if (actionInitiatingUser) {
      ...
      const playerStateChangeMessage = messageUtils.generatePlayerStateChangeMessage(
        userForSocketId.userName,
        videoPlayerForParty
      );
      // Send a message to the party, letting them know that a user started playing the video
      party.sendMessageToParty(
        io,
  ...
  togglePartyWaitingToBeReady: (io, partyId, isNowWaitingForAllUsersToBeReady) => {
    ...
    if (currentlyWaitingToBeReady && !isNowWaitingForAllUsersToBeReady) {
      party.sendMessageToParty(
        io,
        'Everybody is done with buffering !',
        partyId,
        serverUserName
      );
    }
    // If the party was not previously waiting to be ready but now IS waiting for some users to be ready to play
      ...
      party.sendMessageToParty(
        io,
        'Waiting for everyone to be done with buffering ..',
        partyId,
        serverUserName
      );
    }
